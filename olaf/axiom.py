...
def detect_taxonomy_columns(conn: sqlite3.Connection, taxonomy_table: str) -> Tuple[str, str]:
    ...
    # Parent candidates (UPDATED for validated schema)
    parent_candidates = [
...
def detect_triple_columns(conn: sqlite3.Connection, triple_table: str) -> Tuple[str, str, str]:
    ...
    # UPDATED for non_taxonomic_edges(_clean)
    subj_candidates = [
        "subj_canonical_term",
        "subj_text",
        "subject",
...
def compute_domain_range_for_relation(
    ...
    status = "accepted" if (
        ...
        and (coverage_score >= 0.30 if types else True)
    ) else "candidate"
...
def main() -> None:
    ...
    print(f"[INFO] Taxonomy edges loaded: {len(tax_edges)}")
    ...
    # Include type labels as classes
    if types:
